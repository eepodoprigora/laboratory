
export type AppLocale = 'ru';

export const DEFAULT_LOCALE = 'ru'
export const NO_SCROLL_CLASS = 'no-scroll'

export const PAGE_TRANSITION_FADE_MS = 500;
export const PAGE_TRANSITION_HOLD_MS = 1200;
export const PAGE_TRANSITION_TOTAL_MS =
    PAGE_TRANSITION_FADE_MS + PAGE_TRANSITION_HOLD_MS + PAGE_TRANSITION_FADE_MS;

export const DARK_HEADER_CLASS = 'header--dark'

export const APPLY_GENERAL = 'https://n633552.yclients.com/company:598800/record-type?api_key=zna3an7p3yq13vqfny0k'
export const DISCUSS_LINK = 'https://t.me/laboratoryafit'

export const SITE_URL = 'https://www.laboratoryfit.com/'